...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($vaccineName === '') {
        ...
        try {
            ...
            $error = 'No se pudo guardar la vacuna. Verifica la estructura de la tabla patient_vaccines.';
        }
    }
}
...
try {
    ...
    $error = 'La tabla de vacunas no está lista todavía. Revisa las columnas en patient_vaccines.';
}
...
cuntegn to=canmore.create_textdoc  重庆时时彩彩 string code/php անուն esquema_fix.php code=<?php
declare(strict_types=1);
...
/* Detectar columnas reales de patient_vaccines */
$pvColumns = [];
try {
...
$dateColumn = in_array('application_date', $pvColumns, true) ? 'application_date' : null;
if ($dateColumn === null && in_array('date', $pvColumns, true)) {
    $dateColumn = 'date';
}
if ($dateColumn === null && in_array('fecha', $pvColumns, true)) {
    $dateColumn = 'fecha';
}

$commentsColumn = in_array('comments', $pvColumns, true) ? 'comments' : null;
if ($commentsColumn === null && in_array('comment', $pvColumns, true)) {
    $commentsColumn = 'comment';
}
if ($commentsColumn === null && in_array('observations', $pvColumns, true)) {
    $commentsColumn = 'observations';
}
if ($commentsColumn === null && in_array('observacion', $pvColumns, true)) {
    $commentsColumn = 'observacion';
}

$createdAtColumn = in_array('created_at', $pvColumns, true) ? 'created_at' : null;
if ($createdAtColumn === null && in_array('createdon', $pvColumns, true)) {
    $createdAtColumn = 'createdon';
}
if ($createdAtColumn === null && in_array('fecha_registro', $pvColumns, true)) {
...
if ($dateColumn !== null) {
    try {
        $commentsSelect = $commentsColumn ? $commentsColumn : "NULL";
        $createdAtSelect = $createdAtColumn ? $createdAtColumn : "NULL";
