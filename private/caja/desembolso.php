  ...
  try {
    ...
    if ($hasBranchInMov) {
      if ($hasRepInMov) {
        ...
        $st = $pdo->prepare($sql);
        $params = [$sessionId, $branchId, $motivo, $representante, (-abs(round($amount,2))), $userId];
        ...
        $sql = "INSERT INTO cash_movements
                ...
                VALUES
                  (?, ?, 'desembolso', ?, 'efectivo', ?, ?" . ($createdCol ? ", ?" : "") . ")";
        $st = $pdo->prepare($sql);
        $params = [$sessionId, $branchId, $motivo, (-abs(round($amount,2))), $userId];
        if ($createdCol) { $params[] = $nowStr; }
        $st->execute($params);
      }
    } else {
      if ($hasRepInMov) {
        $sql = "INSERT INTO cash_movements
                ...
                VALUES
                  (?, 'desembolso', ?, ?, 'efectivo', ?, ?" . ($createdCol ? ", ?" : "") . ")";
        $st = $pdo->prepare($sql);
        $params = [$sessionId, $motivo, $representante, (-abs(round($amount,2))), $userId];
        if ($createdCol) { $params[] = $nowStr; }
        $st->execute($params);
      } else {
        $sql = "INSERT INTO cash_movements
                ...
                VALUES
                  (?, 'desembolso', ?, 'efectivo', ?, ?" . ($createdCol ? ", ?" : "") . ")";
        $st = $pdo->prepare($sql);
        $params = [$sessionId, $motivo, (-abs(round($amount,2))), $userId];
        if ($createdCol) { $params[] = $nowStr; }
        $st->execute($params);
      }
    }
    ...
    echo "<script>window.open('acuse_desembolso.php?id={$newId}','_blank');window.location.href='" . $_SERVER["PHP_SELF"] . "?ok=1#historial';</script>";
    exit;
...
<head>
  ...
  <link rel="stylesheet" href="/public/assets/css/styles.css?v=50">
  <link rel="stylesheet" href="/assets/css/caja.css?v=50">
  <link rel="stylesheet" href="/public/assets/css/caja.css?v=50">
  <link rel="stylesheet" href="/private/caja/caja.css?v=50">
  ...
  <style>
    ...
    /* Historial: que quede dentro de la card y scrollee */
    .tblWrap{border:1px solid #e6eef7;border-radius:18px;overflow:hidden;background:#fff;}
    .tblScroll{max-height:260px;overflow:auto;}
    .tbl thead th{position:sticky;top:0;z-index:2;background:#eef5ff;}
    .tblScroll::-webkit-scrollbar{width:10px;}
    .tblScroll::-webkit-scrollbar-thumb{background:rgba(0,0,0,.18);border-radius:10px;}
    .tblScroll::-webkit-scrollbar-track{background:rgba(0,0,0,.06);border-radius:10px;}
...
<div class="layout">
  ...
  <div class="panel">
    ...
    <section class="historyCard">
      ...
      <div class="tblWrap"><div class="tblScroll"><table class="tbl">
        <thead>
          <tr>
